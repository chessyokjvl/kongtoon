...
function renderOverviewChart(data) {
    // รวม Keys (YYYY-MM) จากทั้ง 3 กองทุน
...
function renderData() {
    ...
    rawData.forEach(r => {
        ...
        tb.innerHTML += `<tr class="hover:bg-slate-50 border-b border-slate-50"><td class="p-3 md:p-4">${r.date}</td><td class="p-3 md:p-4"><span class="px-2.5 py-1 text-xs font-medium rounded-full ${r.type==='รายรับ'?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}">${r.type}</span></td><td class="p-3 md:p-4"><p class="font-medium text-slate-800">${r.item}</p></td><td class="p-3 md:p-4 text-right font-semibold ${r.type==='รายรับ'?'text-green-600':'text-red-600'}">฿${Number(r.amount).toLocaleString('th-TH')}</td><td class="p-3 md:p-4 text-center">${eviHtml}</td><td class="p-3 md:p-4 text-center">${actionHtml}</td></tr>`;
    });
...
function renderModuleCharts() {
    ...
    rawData.forEach(r => {
        ...
        if(r.type === 'รายรับ') { monthly[d].inc += amt; pInc[r.item] = (pInc[r.item]||0) + amt; }
        else { monthly[d].exp += amt; pExp[r.item] = (pExp[r.item]||0) + amt; }
    });
    ...
    modBarInst = new Chart(document.getElementById('modBarChart').getContext('2d'), { type: 'bar', data: { labels: mKeys, datasets: [{ label: 'รายรับ', data: mIncData, backgroundColor: '#22c55e'}, { label: 'รายจ่าย', data: mExpData, backgroundColor: '#ef4444'}] }, options: { responsive: true, maintainAspectRatio: false } });

    // Line Chart
    if(modLineInst) modLineInst.destroy();
    modLineInst = new Chart(document.getElementById('modLineChart').getContext('2d'), { type: 'line', data: { labels: mKeys, datasets: [{ label: 'กำไร/ส่วนต่าง', data: mProfData, borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.2)', fill: true, tension: 0.3 }] }, options: { responsive: true, maintainAspectRatio: false } });
...
async function submitForm(e) { e.preventDefault(); const btn = document.getElementById('btnSave'); btn.innerText = 'กำลังบันทึก...'; btn.disabled = true; let payload = { module: currentModule, username: currentUser.username, rowIndex: document.getElementById('f_id').value, date: document.getElementById('f_date').value, type: document.getElementById('f_type').value, item: document.getElementById('f_item').value, method: document.getElementById('f_method').value, amount: document.getElementById('f_amount').value, note: document.getElementById('f_note').value }; if(document.getElementById('f_eviType').value === 'file') { const f = document.getElementById('f_file').files[0]; if(f) { if(f.size > 5242880) { Swal.fire('ผิดพลาด', 'ไฟล์ขนาดเกิน 5MB', 'warning'); btn.innerText = 'บันทึกข้อมูล'; btn.disabled = false; return; } payload.fileBase64 = await getBase64(f); payload.fileName = f.name; payload.fileMimeType = f.type; } } else payload.evidenceLink = document.getElementById('f_link').value; try { const res = await (await fetch(CONFIG.API_URL, { method: 'POST', body: JSON.stringify({ action: 'save_tx', payload }) })).json(); if (res.status === 'success') { Swal.fire({title: 'สำเร็จ', text: res.message, icon: 'success', timer: 1500, showConfirmButton: false}); closeModal(); fetchData(); } else Swal.fire('ผิดพลาด', res.message, 'error'); } catch (err) { Swal.fire('Error', 'ไม่สามารถเชื่อมต่อได้', 'error'); } finally { btn.innerText = 'บันทึกข้อมูล'; btn.disabled = false; } }
function editTx(r) { document.getElementById('modalFormTitle').innerText = 'แก้ไขรายการ'; document.getElementById('f_id').value = r.id; document.getElementById('f_date').value = r.date; document.getElementById('f_type').value = r.type; document.getElementById('f_item').value = r.item; document.getElementById('f_method').value = r.method; document.getElementById('f_amount').value = r.amount; document.getElementById('f_note').value = r.note; openModal(); }
