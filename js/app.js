...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for(let i = currYear - 2; i <= currYear + 2; i++) ySel.options.add(new Option(i + 543, i));
    ySel.value = currYear; 
...
function switchModule(mod) {
    ...
    if (mod === 'Overview') {
        ...
        // แสดงปุ่ม Report ทุกคน แต่ปุ่ม Add เฉพาะ Admin
...
function renderOverviewChart(data) {
    ...
    overviewChartInst = new Chart(document.getElementById('overviewChart').getContext('2d'), {
        type: 'bar',
        data: { labels: allKeys, datasets: [ { label: 'กองทุนฯ', data: fundProfit, backgroundColor: '#3b82f6' }, { label: 'ร้านกาแฟ', data: cafeProfit, backgroundColor: '#f59e0b' }, { label: 'ร้านผลิตภัณฑ์', data: shopProfit, backgroundColor: '#10b981' } ] },
...
function renderData() {
    ...
    if (rawData.length > 0) {
        rawData.forEach(r => {
            ...
            if (r.type === 'รายรับ') { if (isCapIn) capIn += amt; else opInc += amt; } 
            else { if (isCapOut) capOut += amt; else opExp += amt; }
...
// ==========================================
// --- ระบบ Report ประจำเดือนแบบ Classic ---
// ==========================================
function openReportModal() { 
    document.getElementById('reportModal').classList.remove('hidden'); 
    document.body.classList.add('overflow-hidden');
    
    // ตั้งชื่อหัวข้อให้ตรงกับ Module
    const titles = { Fund: 'กองทุนฯ', Cafe: 'ร้านกาแฟสุขใจ', Shop: 'ร้านผลิตภัณฑ์' };
    document.getElementById('r-moduleName').innerText = titles[currentModule];
    
    loadMonthlyReport(); // โหลดข้อมูลทันทีเมื่อเปิด
}
...
async function loadMonthlyReport() {
    ...
    document.getElementById('r-monthYear').innerText = `${monthName} ${parseInt(year) + 543}`;

    try {
        const res = await (await fetch(CONFIG.API_URL, { 
            method: 'POST', 
            body: JSON.stringify({ action: 'get_monthly_report', payload: { module: currentModule, month, year } }) 
        })).json();
        ...
        if (res.status === 'success') {
            ...
            document.getElementById('r-broughtForward').innerText = fmt(d.broughtForward);
            document.getElementById('r-expenseGoods').innerText = fmt(d.expenseGoods);
            document.getElementById('r-salesCash').innerText = fmt(d.salesCash);
            document.getElementById('r-expenseSalary').innerText = fmt(d.expenseSalary);
            document.getElementById('r-salesTransfer').innerText = fmt(d.salesTransfer);
            document.getElementById('r-otherExp').innerText = fmt(d.otherExp);
            document.getElementById('r-fundWithdrawal').innerText = fmt(d.fundWithdrawal + d.otherInc);
            document.getElementById('r-totalIncome').innerText = fmt(d.totalIncomeRow);
            document.getElementById('r-totalExpense').innerText = fmt(d.totalExpenseRow);
            document.getElementById('r-totalSales').innerText = fmt(d.totalSales);
            document.getElementById('r-deductExpense').innerText = fmt(d.totalExpenseRow);
            document.getElementById('r-netProfit').innerText = fmt(d.netProfit);
            document.getElementById('r-addBroughtForward').innerText = fmt(d.broughtForward);
            document.getElementById('r-addFund').innerText = fmt(d.fundWithdrawal + d.otherInc);
            document.getElementById('r-totalMoney').innerText = fmt(d.totalMoney);
            document.getElementById('r-cashOnHand').innerText = fmt(d.cashOnHand);
...
function exportPDF() { 
    html2pdf().set({ margin: 0.5, filename: `Report_${currentModule}_${document.getElementById('r-monthYear').innerText}.pdf`.replace(/\s+/g, '_'), html2canvas: { scale: 2 }, jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' } }).from(document.getElementById('printableReport')).save(); 
}

function exportExcel() { 
    XLSX.writeFile(XLSX.utils.table_to_book(document.getElementById("reportTableExport"), {sheet: "Report"}), `Report_${currentModule}_${document.getElementById('r-monthYear').innerText}.xlsx`.replace(/\s+/g, '_')); 
}
...
function updateItemDropdown() {
    ...
    if (currentModule === 'Fund') {
        if (type === 'รายรับ') options = ['ยอดยกมา', 'เงินฝากธนาคาร', 'อื่นๆ'];
        else options = ['ถอนเงินจากธนาคาร', 'อื่นๆ'];
    } else {
        if (type === 'รายรับ') options = ['ยอดยกมา', 'ขายสินค้า', 'ถอนเงินกองทุน', 'อื่นๆ'];
        else options = ['ซื้อของ', 'เงินเดือน', 'คืนเงินกองทุน', 'อื่นๆ'];
    }
...
function toggleOtherItem() {
    ...
    if (select === 'อื่นๆ') { otherInput.classList.remove('hidden'); otherInput.required = true; } 
    else { otherInput.classList.add('hidden'); otherInput.required = false; otherInput.value = ''; }
}
...
async function submitForm(e) { 
    ...
    let payload = { module: currentModule, username: currentUser.username, rowIndex: document.getElementById('f_id').value, date: document.getElementById('f_date').value, type: document.getElementById('f_type').value, item: finalItem, method: document.getElementById('f_method').value, amount: document.getElementById('f_amount').value, note: document.getElementById('f_note').value }; 
...
function editTx(r) { 
    document.getElementById('modalFormTitle').innerText = 'แก้ไขรายการ'; document.getElementById('f_id').value = r.id; document.getElementById('f_date').value = r.date; document.getElementById('f_type').value = r.type; updateItemDropdown(); 
    const select = document.getElementById('f_item_select'); const options = Array.from(select.options).map(o => o.value);
    if (options.includes(r.item)) { select.value = r.item; toggleOtherItem(); } else { select.value = 'อื่นๆ'; toggleOtherItem(); document.getElementById('f_item_other').value = r.item; }
    document.getElementById('f_method').value = r.method; document.getElementById('f_amount').value = r.amount; document.getElementById('f_note').value = r.note; openModal(); 
}
...
function openModal() { document.getElementById('txModal').classList.remove('hidden'); if (!document.getElementById('f_id').value) updateItemDropdown(); }
